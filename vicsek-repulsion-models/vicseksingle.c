...
double update_orientation_2(agent *next_agent, agent *agent_list) {
    ...
    for (int y = -3; y <= 3; y++) {
      ...
      for (int i = 0; i <= BOARD[x_tmp][y_tmp].index; i++) {
          ...
          if (distance <= 2) {
            ...
            if (fabs(agent_list[neighbor_number].y_coord - next_agent->y_coord) >= BOARD_LENGTH - 1) {
            ...
            //double x1 = next_agent->x_coord - (x_comp / magnitude);
            //double y1 = next_agent->y_coord - (y_comp / magnitude);
            //double x2 = agent_list[neighbor_number].x_coord + (x_comp / magnitude);
            //double y2 = agent_list[neighbor_number].y_coord + (y_comp / magnitude);

            //double x_overlap = x1 - x2; 
            //double y_overlap = y1 - y2;
            //printf("Check out this fucking delta: %f\n", distance);
            //stress = stress + ((sqrt((x_overlap * x_overlap) + (y_overlap * y_overlap)) - 2) / 2);
            stress = (2 - distance) / 2;
          }
  ...
  if (neighbor_count > 0) {
    ...
    //double magnitude_repulse = sqrt((repulse_x * repulse_x) + (repulse_y * repulse_y));
